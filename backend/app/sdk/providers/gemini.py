...
class GeminiProvider(BaseProvider):
    ...
    def _convert(self, messages: List[Dict[str, str]]) -> tuple[List[Dict], str | None]:
        ...
        for m in messages:
            if m["role"] == "system":
                ...
                result.append({"role": "user", "parts": [m["content"]]})
    ...
    async def complete(
        ...
        gen_model = genai.GenerativeModel(
            model_name=model, system_instruction=sys_inst
        )

        try:
            # Pass full conversation history, not just last message
            response = await gen_model.generate_content_async(contents=conv)
        except google_exceptions.ResourceExhausted as e:
            # Rate limit — extract retry delay if available
            retry_msg = str(e)
            retry_sec = None
            try:
                err = json.loads(e.message) if hasattr(e, "message") else {}
            except Exception:
                err = {}
            ...
            for d in details:
                if d.get("@type", "").endswith("RetryInfo"):
                    ...
                    if rd and rd.endswith("s"):
                        retry_sec = int(rd[:-1])
            wait = f" Retry in {retry_sec}s." if retry_sec else ""
            raise RuntimeError(
                f"Gemini free tier quota exhausted.{wait} "
                f"Consider waiting for quota reset or using Groq instead."
            ) from e
    ...
    async def stream(
        ...
        try:
            response = await gen_model.generate_content_async(
                contents=conv, stream=True
            )
        except google_exceptions.ResourceExhausted as e:
            raise RuntimeError(
                "Gemini free tier quota exhausted. "
                "Consider waiting for quota reset or using Groq instead."
            ) from e
